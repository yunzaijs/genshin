...
import { MysInfo, GSCfg } from '@yunzaijs/mys';
...
class RoleDetail extends base {
    ...
    async getDetail() {
        ...
        if (!(await this.checkImg(avatar.name)))
            return false;
        /** 截图数据 */
        let data = {
            quality: 80,
    ...
    async getAvatar(data) {
        ...
        if (this.e.roleId == '20000000') {
            if (avatars['10000007'])
                this.e.roleId = '10000007';
            if (avatars['10000005'])
                this.e.roleId = '10000005';
        }
    ...
    async checkImg(name) {
        if (fs.existsSync(`${this.path}${name}1.png`))
            return true;
